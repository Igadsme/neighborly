...
import { Badge, Button, Icon, TabBar } from '../components/ui'
...
export default function Jobs() {
  ...
  const filtered = jobs.filter(j => {
    ...
    if (activeLevel !== 'Any level' && !j.level.includes(activeLevel)) return false
    if (remoteFilter === 'remote' && j.remote !== 'Remote') return false
    if (remoteFilter === 'hybrid' && j.remote !== 'Hybrid') return false
    if (remoteFilter === 'onsite' && j.remote !== 'On-site') return false
  ...
  const toggleApply = (id: string) => setApplied(prev => prev.includes(id) ? prev : [...prev, id])
  const toggleSave = (id: string) => setSaved(prev => prev.includes(id) ? prev.filter(i => i !== id) : [...prev, id])
      ...
      <div className="bg-[#1B2A4A] text-white py-12 px-4 md:px-6">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="flex gap-3 max-w-2xl">
            ...
            <Button variant="secondary" size="md">Search jobs</Button>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 md:px-6 py-6">
        ...
        <div className="flex gap-6">
          ...
          <div className="flex-1 md:w-80 md:flex-shrink-0">
            <p className="text-sm text-[#8A9AB5] mb-4"><span className="font-bold text-[#1B2A4A]">{filtered.length}</span> jobs in Atlanta</p>
                ...
                >
                  ...
                  <div className="flex items-start gap-3 mb-3">
                    <div className="w-10 h-10 rounded-xl overflow-hidden flex-shrink-0 bg-[#F5F4EF]">
                      <img src={`https://images.unsplash.com/${job.logo}?w=80&h=80&fit=crop&auto=format`} alt={job.company} className="w-full h-full object-cover" />
                    </div>
                  ...
                  <div className="flex flex-wrap gap-1.5 mb-2">
                    ...
                    <span className={`inline-flex items-center text-xs font-semibold px-2 py-0.5 rounded-md ${remoteColors[job.remote]}`}>{job.remote}</span>
              ...
              <div className="bg-white rounded-3xl border border-[#E8E6DF] p-6 sticky top-24">
                ...
                <div className="flex items-start gap-4 mb-5">
                  <div className="w-14 h-14 rounded-2xl overflow-hidden bg-[#F5F4EF]">
                    <img src={`https://images.unsplash.com/${selectedJob.logo}?w=100&h=100&fit=crop&auto=format`} alt={selectedJob.company} className="w-full h-full object-cover" />
                  </div>
                  <div className="flex-1">
                    ...
                    <div className="flex items-center gap-2 mt-1">
                      ...
                      <span className={`inline-flex items-center text-xs font-semibold px-2 py-0.5 rounded-md ${remoteColors[selectedJob.remote]}`}>{selectedJob.remote}</span>
                    </div>
                  </div>
                  <button onClick={() => toggleSave(selectedJob.id)} className="w-10 h-10 rounded-full bg-[#F5F4EF] flex items-center justify-center hover:bg-[#EEF0F5] transition-colors">
                ...
                <div className="mb-5">
                  ...
                  <p className="text-sm text-[#5C6E8A] leading-relaxed">
                    We're looking for a talented {selectedJob.title} to join our growing team in {selectedJob.location}. This is a {selectedJob.type.toLowerCase()} position with {selectedJob.remote.toLowerCase()} flexibility. You'll be working on impactful projects and collaborating with a small, motivated team.
                  </p>
                </div>
                ...
                <div className="mb-5">
                  ...
                  <ul className="space-y-2 text-sm text-[#5C6E8A]">
                    <li className="flex items-start gap-2"><span className="text-[#2D6A4F] mt-0.5">•</span>Lead development of features with a strong focus on quality and performance</li>
                    <li className="flex items-start gap-2"><span className="text-[#2D6A4F] mt-0.5">•</span>Collaborate with product and design teams to shape requirements</li>
                    <li className="flex items-start gap-2"><span className="text-[#2D6A4F] mt-0.5">•</span>Participate in code reviews and contribute to engineering best practices</li>
                    <li className="flex items-start gap-2"><span className="text-[#2D6A4F] mt-0.5">•</span>Mentor junior team members where applicable</li>
                  </ul>
                </div>
                ...
                <div className="flex gap-3">
                  ...
                  >
                    {applied.includes(selectedJob.id) ? '✓ Applied!' : 'Apply now →'}
                  </Button>
