...
- Messaging: list conversations, list/send messages, mark read, for existing participants. Socket.IO namespace `/realtime` requires a JWT and emits `message.created` after a participant sends. The signed-in nav badge counts unread threads from `GET /conversations` (caller `lastReadAt` compared with the latest message). It is not a notification list. Signed-out, an empty or fully read inbox, and 401 hide the badge.
